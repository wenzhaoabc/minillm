...
from minillm.dataset.lm_dataset import PretrainDataset
...
if __name__ == "__main__":
    ...
    if not script_args.data_path:
        script_args.data_path = "../dataset/pretrain_hq.jsonl"
    if script_args.from_weight is None:
        script_args.from_weight = "none"
    if train_args.accumulation_steps == 1:
        train_args.accumulation_steps = 8
    ...
    wandb_run_name = (
        args.run_name
        or f"MiniLLM-Pretrain-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
    )
    wandb = init_wandb_run(args, ckp_data, wandb_run_name, lm_config)
    save_run_metadata(
        args.output_dir, lm_config, args, extra={"stage": "pretrain"}, wandb=wandb
    )
    ...
    model, tokenizer = init_model(
        ...
        save_dir=args.save_dir,
        load_dir=args.load_dir or args.load_from,
        device=args.device,
    )
    if args.use_compile:
