...
def train_epoch(epoch, loader, iters, teacher_model, lm_config_student, start_step=0, wandb=None, alpha=0.0, temperature=1.0):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters - 1) and is_main_process():
            ...
            lm_checkpoint(lm_config_student, weight=args.save_weight, model=model, optimizer=optimizer, scaler=scaler, epoch=epoch, step=step, wandb=wandb, save_dir=args.checkpoint_dir, save_model_artifact=False, save_total_limit=args.save_total_limit)
            model.train()
            del state_dict
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniLLM Knowledge Distillation")
    ...
    parser.add_argument("--save_total_limit", type=int, default=None, help="最多保留的checkpoint数量，默认不限制")
    ...
    lm_config_student = build_lm_config_from_args(args, overrides={"hidden_size": args.student_hidden_size, "num_hidden_layers": args.student_num_layers})
    lm_config_teacher = build_lm_config_from_args(args, overrides={"hidden_size": args.teacher_hidden_size, "num_hidden_layers": args.teacher_num_layers})
    ckp_data = lm_checkpoint(lm_config_student, weight=args.save_weight, save_dir=args.checkpoint_dir) if args.from_resume==1 else None
    ...
    # ========== 4. 配wandb ==========
    wandb_run_name = f"MiniLLM-Distill-S{args.student_hidden_size}T{args.teacher_hidden_size}-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
    wandb = init_wandb_run(args, ckp_data, wandb_run_name, lm_config_student)
    save_run_metadata(
        args.save_dir,
        lm_config_student,
        args,
        extra={"stage": "distillation", "teacher_config": lm_config_teacher.to_dict()},
        wandb=wandb,
    )
    
    # ========== 5. 定义学生和教师模型 ==========
    model, tokenizer = init_model(lm_config_student, args.from_student_weight, save_dir=args.save_dir, device=args.device)
    if args.use_compile == 1:
        model = torch.compile(model)
        Logger('torch.compile enabled')
    Logger(f'学生模型总参数量：{sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    teacher_model, _ = init_model(lm_config_teacher, args.from_teacher_weight, save_dir=args.save_dir, device=args.device)
    teacher_model.eval()
    teacher_model.requires_grad_(False)
    ...
    if dist.is_initialized() and dist.get_world_size() > 1:
        ...
        model = DistributedDataParallel(
            ...
            find_unused_parameters=bool(getattr(lm_config_student, "use_moe", False)),
        )
