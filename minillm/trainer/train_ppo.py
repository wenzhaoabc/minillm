...
from minillm.dataset.lm_dataset import RLAIFDataset
from minillm.trainer.configs import (
    PPOConfigArgs,
    parse_config_groups,
    namespace_from_configs,
)
from minillm.trainer.trainer_utils import (
    Logger,
    is_main_process,
    lm_checkpoint,
...
# 自定义的Critic模型，继承自MiniLLMForCausalLM
...
def ppo_train_epoch(
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        last_indices = (
            full_mask * torch.arange(full_mask.size(1), device=gen_out.device)
        ).argmax(dim=1)
        values = values_seq[
            torch.arange(values_seq.size(0), device=values_seq.device), last_indices
        ]  # [B]
        ...
        loss = (
            policy_loss + args.vf_coef * value_loss + args.kl_coef * kl_ref + aux_loss
        ) / args.accumulation_steps  # scalar
        loss.backward()
...
if __name__ == "__main__":
    ...
    args.checkpoint_dir = resolve_checkpoint_dir(
    ...
    wandb_run_name = (
        args.run_name
        or f"MiniLLM-PPO-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
    )
    wandb = init_wandb_run(args, ckp_data, wandb_run_name, lm_config)
    save_run_metadata(
        args.output_dir, lm_config, args, extra={"stage": "ppo"}, wandb=wandb
    )
    ...
    # Critic模型
    critic_bootstrap_model, _ = init_model(
        lm_config,
        base_weight,
        tokenizer_path=args.tokenizer_path,
        save_dir=args.save_dir,
    ...
    if ckp_data:
        actor_model.load_state_dict(
            load_model_state(ckp_data["model_path"], map_location="cpu")
        )
