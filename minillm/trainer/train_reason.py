...
def train_epoch(epoch, loader, iters, tokenizer, lm_config, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters - 1) and is_main_process():
            ...
            lm_checkpoint(lm_config, weight=args.save_weight, model=model, optimizer=optimizer, scaler=scaler, epoch=epoch, step=step, wandb=wandb, save_dir=args.checkpoint_dir, save_model_artifact=False, save_total_limit=args.save_total_limit)
            model.train()
            del state_dict
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniLLM Reasoning Distillation")
    ...
    # ========== 4. 配wandb ==========
    wandb_run_name = f"MiniLLM-Reasoning-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LR-{args.learning_rate}"
    wandb = init_wandb_run(args, ckp_data, wandb_run_name, lm_config)
    save_run_metadata(args.save_dir, lm_config, args, extra={"stage": "reason"}, wandb=wandb)
