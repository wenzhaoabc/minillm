...
# MoE模型需使用此函数转换
def convert_torch2transformers_minillm(torch_path, transformers_path, dtype=torch.float16):
    MiniLLMConfig.register_for_auto_class()
    MiniLLMForCausalLM.register_for_auto_class("AutoModelForCausalLM")
    lm_model = MiniLLMForCausalLM(lm_config)
    ...
    print(f"模型已保存为 Transformers-MiniLLM 格式: {transformers_path}")
...
if __name__ == '__main__':
    lm_config = MiniLLMConfig(hidden_size=512, num_hidden_layers=8, max_seq_len=8192, use_moe=False)
    torch_path = f"../out/full_sft_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth"
    transformers_path = '../MiniLLM2-Small'
